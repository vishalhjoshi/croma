			...
			with transaction.atomic():
				...
				if self.request.method == "POST" and self.request.is_ajax():
					...
					response = {
						...
						'url' : sale_hrd_obj.get_absolute_sale_detail_url(),
					}
...
def RetrieveSale(request, pk = None):
	...
	saleInvhrd_query = get_object_or_404(SalesInvHrd, id = pk, session_id = current_session)
	doc_no = saleInvhrd_query.doc_no
	saleInvhrd_form = SalesInvHrdForm(request.POST or None, instance = saleInvhrd_query)
	...
	context = {
			...
			"doc_no": doc_no,
			"saleInvhrd_query": saleInvhrd_query,
...
class UpdateSale(UpdateView, LoginRequiredMixin):
	...
	def get(self, request, pk = None):
		...
		saleInvDtlForm = SalesInvDtlForm(request.POST or None)
		#Get the sale inv Query
		saleInvhrd_query = get_object_or_404(SalesInvHrd, id = pk)
		doc_no = saleInvhrd_query.doc_no
			...
			with transaction.atomic():
				if self.request.method == "POST" and self.request.is_ajax():
					...
					response = {
								'status': 1,
								'message': "Successfully Saved",
								'url' : SaleInvHrd_instance.get_absolute_sale_detail_url(),
							}
					return JsonResponse(response, status = 200)
...
def DeleteSale(request):
	...
	if request.method == "POST" and request.is_ajax():
		...
		try:
			...
			url = next_sale_inv.get_absolute_sale_detail_url()
		except:
			next_sale_inv = None
			url = "/sales/create"
...
def ViewInvoice(request):
	...
	if request.method == "POST" and request.is_ajax():
		...
		try:
			...
			file_path = os.path.join(settings.BASE_DIR, 'templates\print_inv.txt')
...
def SearchInv(request):
	...
	if request.method == "POST" and request.is_ajax():
		...
		if SalesInvHrd.objects.filter(session_id = current_session, doc_no = inv_no).exists():
			...
			response = {"status": 1, "msg" : sale_inv_obj.get_absolute_sale_detail_url()}
		else:
