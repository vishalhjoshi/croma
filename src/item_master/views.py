		...
		try:
			if self.request.method == "POST" and self.request.is_ajax():
				...
				if batch_serialzer_instance.is_valid():
					...
					logger.error(errors)
					return JsonResponse(errors, status = 400)
				response = {'url' : item_instance.get_absolute_item_detail_url()}
...
def RetrieveItem(request, pk = None):
	...
	item_query = get_object_or_404(Item, id = pk)
	...
	item_form = ItemMasterForm(request.POST or None, instance=item_query)
	try:
		next_item = Item.objects.filter(pk__gt=item_query.id).order_by('pk').first()
	except:
		next_item = None
		logger.warning("No next item found")
	try:
		prev_item = Item.objects.filter(pk__lt=item_query.id).order_by('-pk').first()
	except:
		prev_item = None
		...
		try:
			if self.request.method == "POST" and self.request.is_ajax():
				...
				if serializer_item.is_valid():
					...
					transaction.savepoint_commit(sid)
					response = {'url' : item.get_absolute_item_detail_url(),}
					return JsonResponse(response, status = 200)
				else:
...
def DeleteItem(request):
	if request.method == "POST" and request.is_ajax():
		...
		try:
			...
			url = next_pur_inv.get_absolute_item_detail_url()
		except:
			next_item = None
			url = "/item/create"
...
def search_item(request):
	if request.method == "GET" and request.is_ajax():
		...
		for item in item_list:
			...
			item_obj['url'] = str(item.get_absolute_item_detail_url())
...
def get_item_batches(request):
	if request.method == "GET" and request.is_ajax():
		...
		data = {
			...
			"item_query": json.dumps(item_json_Arr,  default=date_handler),
		}
		return JsonResponse(data, status = 200)
