		...
		try:
			with transaction.atomic():
				if self.request.method == "POST" and self.request.is_ajax():
					...
					response = {
						'status': 1,
						'url' : pur_hrd_obj.get_absolute_purchase_detail_url(),
					}
...
def RetrievePurchase(request, pk = None):
	...
	context = {
			...
			"doc_no": doc_no,
			"purInvhrd_query": purchaseInvhrd_query,
			"hrd_form" : PurInvHrdForm,
			"dtl_form" : PurInvDtlForm,
			"prev": prev_pur_inv,
			"next":next_pur_inv,
			...
			with transaction.atomic():
				if self.request.method == "POST" and self.request.is_ajax():
					...
					response = {
							...
							'url' : PurchaseInvHrd_instance.get_absolute_purchase_detail_url(),
					}
...
def DeletePurchase(request):
	...
	if request.method == "POST" and request.is_ajax():
		...
		try:
			...
			url = next_pur_inv.get_absolute_purchase_detail_url()
		except Exception as e:
			next_sale_inv = None
			url = "/purchase/create"
...
def SearchInv(request):
	...
	if request.method == "POST" and request.is_ajax():
		...
		if PurchaseInvHrd.objects.filter(session_id = current_session, doc_no = inv_no).exists():
			...
			response = {"status": 1, "msg" : purchase_inv_obj.get_absolute_purchase_detail_url()}
		else:
